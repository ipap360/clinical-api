...
public class RoomAvailabilityDao {
    ...
    public List<Map<String, Object>> list(LocalDate d1, LocalDate d2) {
        ...
        final String sql = "SELECT D.ID AS ID, P.GENDER AS GENDER, COALESCE(COUNT(V.ID), 0) AS CNT FROM DATES D INNER JOIN ADMISSIONS V ON D.ID >= V.ADMISSION_DATE AND D.ID < V.RELEASE_DATE INNER JOIN PATIENTS P ON P.ID = V.PATIENT_ID WHERE D.ID BETWEEN :FROM_DATE AND :TO_DATE GROUP BY D.ID, P.GENDER";
