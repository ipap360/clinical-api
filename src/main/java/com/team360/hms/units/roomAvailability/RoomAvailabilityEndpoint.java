...
import com.team360.hms.units.patients.Gender;
...
public class RoomAvailabilityEndpoint {
    ...
    public Response get(
        ...
        return Response.ok().entity(process(from, to)).build();
    ...
    public Map<LocalDate, RoomAvailability> process(LocalDate d1, LocalDate d2) {
        try {
            ...
            List<Map<String, Object>> map = RoomAvailabilityDao.list(d1, d2);
            map.forEach((v) -> {
            ...
            throw new DomainException("Please select a valid period");
        }
    }
