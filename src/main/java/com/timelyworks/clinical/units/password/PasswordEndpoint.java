...
import com.timelyworks.clinical.common.values.Message;
...
public class PasswordEndpoint {
    ...
    public Response forgot(@HeaderParam(HttpHeaders.USER_AGENT) String userAgent,
        ...
        List<String> statuses = Arrays.asList(PasswordResetRequest.Status.PENDING.name(), PasswordResetRequest.Status.VERIFIED.name(), PasswordResetRequest.Status.SENT.name());
        ...
        Integer count = passwordDao.countBySameIpStatusCreation(ip, PasswordResetRequest.Status.PENDING.name(), since);
        if (count > 2) {
        ...
        req.setStatus(PasswordResetRequest.Status.PENDING);
        WebUtl.db(crc).upsert(req);

        final UserDao userDao = new UserDao();
        Optional<Integer> userId = userDao.findByUsername(form.getEmail());
        if (userId.isPresent()) {
            req.setStatus(PasswordResetRequest.Status.VERIFIED);
            WebUtl.db(crc).upsert(req);
            ...
            try {
                ...
                req.setStatus(PasswordResetRequest.Status.SENT);
                WebUtl.db(crc).upsert(req);
            } catch (Exception e) {
    ...
    public Response reset(ResetPasswordForm form) {
        ...
        req.setStatus(PasswordResetRequest.Status.COMPLETED);
